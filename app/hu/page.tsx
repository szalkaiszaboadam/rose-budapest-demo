...
import { motion, AnimatePresence, useScroll, useTransform, useMotionValueEvent, Variants, useSpring } from "framer-motion";
import RoseNavbar from "@/components/RoseNavbar";

// --- ROSE ADATOK ---
const DISHES = [
  { name: "Rose Signature", desc: "A ház ikonikus koktélja prémium ginnel, rózsavízzel és friss málnával.", info: "Lágy, virágos jegyek és frissítő citrusok tökéletes harmóniája. Kézműves rózsavízzel és ehető virágszirmokkal tálalva a tökéletes eleganciáért.", img: "/1.jpg" },
  { name: "Smoked Negroni", desc: "Klasszikus Negroni egy csipetnyi füsttel és érlelt fűszerekkel.", info: "A hagyományos receptet tölgyfa füsttel bolondítjuk meg, ami mély, karakteres ízvilágot kölcsönöz az italnak. Igazi ínyenceknek.", img: "/2.jpg" },
  { name: "Truffle Labneh", desc: "Szarvasgombás krémsajt pirított pisztáciával és gránátalmával.", info: "Selymesen lágy, házi készítésű labneh, melyet prémium szarvasgomba olajjal, ropogós pisztáciával és friss gránátalmamagokkal teszünk felejthetetlenné.", img: "/3.jpg" },
  { name: "Spicy Margarita", desc: "Tequila alapú frissítő egy csipetnyi jalapeño pikánsságával.", info: "A citrusos frissességet és a prémium tequilát egy leheletnyi jalapeño chili teszi izgalmassá. Sós-fűszeres peremmel tálalva.", img: "/4.jpg" },
  { name: "Bosphorus Breeze", desc: "Vodka, uborka, menta és bodza – a nyári éjszaka íze.", info: "Könnyed, ropogós és végtelenül frissítő. Az uborka és a menta hűsítő hatását a bodza édeskés virágossága egészíti ki.", img: "/5.jpg" },
  { name: "Kacsa Rillette Meze", desc: "Lassan sült kacsahús ropogós pitával és füge lekvárral.", info: "Omlós, fűszeres kacsahús, melyet édes-pikáns füge lekvárral és frissen sült, meleg anatóliai pitával kínálunk megosztásra az italok mellé.", img: "/6.jpg" },
  { name: "Espresso Martini", desc: "Sötét, krémes és energikus – prémium vodka és friss espresso.", info: "Az este beindítója. Gondosan kiválasztott, frissen pörkölt kávéból készült espresso, selymes kávélikőr és tiszta vodka tökéletes aránya.", img: "/7.jpg" }
];
...
// --- ÚJ: Popup időzítése a preloader UTÁN (Gyorsítva!) ---
  ...
  // --- ERŐSÍTETT SZÍNES BLUR GÖMBÖK MOZGÁSA ---
  const orb1Y = useTransform(scrollY, [0, 4000], ["0%", "80%"]);
  const orb2Y = useTransform(scrollY, [0, 4000], ["0%", "-60%"]);
  const orb3Y = useTransform(scrollY, [0, 4000], ["0%", "50%"]);
  const orb4Y = useTransform(scrollY, [0, 4000], ["0%", "-40%"]);

  const turkizRef = useRef<HTMLElement>(null);
  const { scrollYProgress: turkizScrollYProgress } = useScroll({ target: turkizRef, offset: ["start end", "end start"] });
  ...
  return (
    <div className="bg-[#050505] overflow-x-clip selection:bg-[#E7918A] selection:text-white font-sans text-white relative">
      ...
      {/* --- ERŐSÍTETT PARALLAX HÁTTÉR GÖMBÖK --- */}
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        <motion.div style={{ y: orb1Y }} className="absolute top-[5%] left-[-15%] w-[45vw] h-[45vw] rounded-full bg-[#E7918A]/25 blur-[160px]" />
        <motion.div style={{ y: orb2Y }} className="absolute top-[35%] right-[-10%] w-[55vw] h-[55vw] rounded-full bg-[#E7918A]/20 blur-[180px]" />
        <motion.div style={{ y: orb3Y }} className="absolute top-[65%] left-[5%] w-[40vw] h-[40vw] rounded-full bg-[#E7918A]/20 blur-[150px]" />
        ...
        {showEventPopup && (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 w-full p-6 md:p-8 flex flex-col z-10">
              ...
              <p className="text-white font-sans text-[13px] md:text-[14px] mb-6 drop-shadow-md">
...
{/* STICKY ASZTALFOGLALÁS GOMB (Javított, konfliktusmentes animáció) */}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial={{ opacity: 0, backdropFilter: "blur(0px)" }} animate={{ opacity: 1, backdropFilter: "blur(20px)" }} exit={{ opacity: 0, backdropFilter: "blur(0px)" }} transition={{ duration: 0.4 }} className="fixed inset-0 z-[45] bg-[#050505]/95 flex flex-col items-center justify-center">
            <motion.div variants={staggerContainer} initial="hidden" animate="visible" exit="hidden" className="flex flex-col items-center gap-8">
              {[
                { label: "Koktélok & Mezzék", href: "#about" }, { label: "Kínálatunk", href: "#menu" }, { label: "Miért Mi?", href: "#why-us" },
                { label: "Események", href: "/hu/events" }, { label: "Vélemények", href: "#reviews" }, { label: "Kapcsolat", href: "#reservation" }
              ].map((item, idx) => (
      ...
      {/* DINAMIKUS VIDEÓS HERO SZEKCIÓ */}
      <section className="relative min-h-screen lg:min-h-screen w-full flex flex-col justify-center items-center px-4 lg:px-16 overflow-hidden bg-black">
        <motion.div style={{ y: heroBgY, scale: 1.15 }} className="absolute inset-0 w-full h-full pointer-events-none">
          <video autoPlay loop muted playsInline className="w-full h-full object-cover opacity-80">
            <source src="/rose-hero-video.mp4" type="video/mp4" />
          </video>
        ...
        <div className="relative z-20 max-w-[90rem] mx-auto w-full flex flex-col items-center text-center mt-20 md:mt-28 pb-32 lg:pb-24">
          <motion.div initial="hidden" animate={!isLoading ? "visible" : "hidden"} variants={staggerContainer} className="flex flex-col items-center">
            ...
            <motion.div variants={fadeUpReveal} className="flex flex-col sm:flex-row gap-4 sm:gap-6 mt-12 md:mt-16">
              ...
              <a href="/hu/menu" className="inline-flex items-center justify-center bg-transparent backdrop-blur-sm border border-white/30 text-white px-10 py-4 uppercase font-sans tracking-[0.2em] text-[11px] md:text-[12px] font-bold hover:bg-white hover:text-black transition-all duration-300 text-center hover:shadow-[0_0_20px_rgba(255,255,255,0.4)] rounded-sm min-w-[220px]">
        ...
        <div className="absolute bottom-10 md:bottom-14 left-0 w-full z-20 px-6 lg:px-14">
          <motion.div initial="hidden" animate={!isLoading ? "visible" : "hidden"} variants={staggerContainer} className="max-w-[90rem] mx-auto flex flex-col md:flex-row justify-between items-center md:items-end gap-6">
            <motion.div variants={fadeUpReveal} className="max-w-md text-center md:text-left order-2 md:order-1">
              <p className="text-white/90 font-sans text-[14px] md:text-[16px] leading-relaxed drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">Prémium signature koktélok, modern mezzék és utánozhatatlan éjszakai lüktetés a Parlament szomszédságában.</p>
            </motion.div>
            <motion.a href="https://www.google.com/search?q=ROSE+SHISHA+%26COCKTAIL+BAR+Vélemények" target="_blank" rel="noopener noreferrer" variants={fadeUpReveal} className="order-1 md:order-2 flex flex-col items-center md:items-end gap-1.5 hover:opacity-100 opacity-90 transition-opacity duration-300 cursor-pointer drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">
              <div className="flex items-center gap-3">
                <span className="font-serif text-white text-[28px] md:text-3xl font-bold leading-none">4,7</span>
                <div className="flex gap-1 text-[#E7918A] drop-shadow-[0_0_10px_rgba(231,145,138,0.8)]">
                  {[...Array(5)].map((_, i) => <svg key={i} className="w-5 h-5 md:w-6 md:h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>)}
                </div>
              </div>
      ...
      {/* KÍNÁLATUNK SZEKCIÓ */}
      <section id="menu" ref={horizontalRef} className="relative h-[300vh] bg-transparent w-full">
        <div className="sticky top-0 flex h-screen w-full items-center overflow-hidden">
          <motion.div ref={carouselRef} style={{ x: horizontalX, willChange: "transform" }} className="flex items-center gap-8 md:gap-14 px-6 md:px-20 w-max">
            ...
            {DISHES.map((dish, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.6, delay: 0.1 * (idx % 3) }} className="w-[85vw] sm:w-[50vw] md:w-[450px] lg:w-[550px] flex flex-col group flex-shrink-0 cursor-pointer relative z-10">
                ...
                <div className="mt-8 transition-transform duration-500">
                  <h3 className="text-2xl md:text-3xl font-serif text-white uppercase tracking-wide group-hover:text-[#E7918A] transition-colors drop-shadow-md">{dish.name}</h3>
      ...
      <section id="why-us" className="relative py-28 lg:py-40 bg-transparent overflow-hidden">
        <div className="relative z-10 max-w-[90rem] mx-auto px-6 lg:px-12">
          <div className="text-center flex flex-col items-center mb-24">
            <motion.h4 variants={fadeUpReveal} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-[#E7918A] font-sans text-[11px] md:text-[13px] font-bold uppercase tracking-[0.25em] mb-4 md:mb-5 drop-shadow-[0_0_8px_rgba(231,145,138,0.4)]">Miért Mi?</motion.h4>
      ...
      <section className="pt-24 pb-32 lg:pt-32 lg:pb-40 bg-transparent overflow-hidden relative z-10">
        <div className="max-w-[90rem] mx-auto px-6 lg:px-12 flex flex-col items-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="flex flex-col text-center items-center gap-4 mb-16">
            <motion.h4 variants={fadeUpReveal} className="text-[#E7918A] font-sans text-[11px] md:text-[13px] font-bold uppercase tracking-[0.25em] mb-4 md:mb-5">Vendégeink</motion.h4>
            <motion.h2 variants={fadeUpReveal} className="text-4xl md:text-5xl font-serif text-white uppercase">KIK JÁRTAK NÁLUNK?</motion.h2>
            <motion.p variants={fadeUpReveal} className="text-white/60 max-w-xl text-[14px] md:text-[15px] font-sans leading-relaxed mt-4">Büszkék vagyunk rá, hogy a ROSE élményt az évek során már számos hazai és nemzetközi ismert személyiség is átélte nálunk.</motion.p>
          </motion.div>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={staggerContainer} className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 w-full mb-16">
            {VIP_IMAGES.map((item, idx) => (
              <motion.a variants={fadeUpReveal} href={item.url} target="_blank" rel="noopener noreferrer" key={idx} className="group aspect-square relative overflow-hidden bg-[#111111] block cursor-pointer rounded-sm border border-white/5">
                ...
                <img src={`/rose-vendeg${item.id}.jpg`} alt={`VIP Guest ${idx + 1}`} className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700 ease-out opacity-80 group-hover:opacity-100" />
              </motion.a>
            ))}
          </motion.div>
      ...
      <section id="reviews" className="py-28 lg:py-40 bg-transparent overflow-hidden relative z-10">
        <div className="relative z-10 max-w-[90rem] mx-auto px-6 lg:px-12 mb-20 text-center">
          <motion.h4 initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="text-[#E7918A] font-sans text-[11px] md:text-[13px] font-bold uppercase tracking-[0.25em] mb-4 md:mb-5">Vélemények</motion.h4>
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.1 }} className="text-4xl lg:text-6xl font-serif text-white tracking-tight uppercase">RÓLUNK MONDTÁK</motion.h2>
        </div>
        <div className="relative z-10 w-full overflow-hidden flex items-center py-6">
          <motion.div animate={{ x: ["0%", "-50%"] }} transition={{ ease: "linear", duration: 95, repeat: Infinity }} className="flex w-max gap-8 md:gap-12 px-6">
            {[...REVIEWS, ...REVIEWS].map((review, idx) => (
              <div key={idx} className="bg-[#111111]/80 backdrop-blur-md border border-white/10 p-8 md:p-12 rounded-sm relative shadow-[0_15px_30px_rgba(231,145,138,0.05)] w-[85vw] sm:w-[500px] md:w-[600px] flex-shrink-0 flex flex-col justify-between overflow-hidden group">
                <div className="absolute bottom-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-[#E7918A]/50 to-transparent"></div>
                <div className="relative z-10">
                  <div className="flex gap-2 text-[#E7918A] mb-8">
                    {[...Array(5)].map((_, i) => <svg key={i} className="w-5 h-5 md:w-6 md:h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>)}
                  </div>
                  <p className="text-white/80 font-sans text-[17px] md:text-[20px] leading-relaxed mb-10 font-normal italic">"{review.text}"</p>
                </div>
      ...
      <div className="w-full bg-transparent text-white flex flex-col relative z-20">
          ...
          <div className="relative z-10 w-full max-w-[90rem] mx-auto px-6 lg:px-12 text-white">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={staggerContainer} className="flex flex-col items-center">
              <div className="flex flex-col items-center text-center mb-16 w-full">
                <motion.h4 variants={fadeUpReveal} className="text-[#E7918A] font-sans text-[11px] md:text-[13px] font-bold uppercase tracking-[0.25em] mb-4 md:mb-5 drop-shadow-[0_0_8px_rgba(231,145,138,0.4)]">Kapcsolat</motion.h4>
        ...
        <section ref={turkizRef} className="relative z-10 py-16 lg:py-32 overflow-hidden min-h-[40vh] flex items-center bg-[#050505]">
        ...
        <footer className="relative z-10 py-8 lg:py-10 bg-[#050505]">
          <div className="max-w-[90rem] mx-auto px-6 lg:px-12 flex flex-col xl:flex-row justify-between items-center gap-6 xl:gap-4 w-full">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center xl:justify-end gap-3 text-[10px] md:text-[11px] font-sans tracking-[0.18em] uppercase text-white/30 w-full xl:w-auto">
              {/* --- LÁBLÉC NÉV CSERÉLVE --- */}
              <p className="whitespace-nowrap">&copy; {new Date().getFullYear()} ROSE MEZE & COCKTAIL BAR</p>
            </div>
          </div>
        </footer>
      </div>
